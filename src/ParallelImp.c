	...
	jobject jobj )
{
	...
	fd = get_java_var( env, jobj,"fd","I" );
	sendEvent = (*env)->GetMethodID( env, jclazz, "sendEvent", "(IZ)V" );
	interrupt = (*env)->GetMethodID( env, jclazz, "checkMonitorThread", 
		"()Z" );
	...
	while( !interrupted )
	{
		...
		while (ret < 0 && errno == EINTR);
		if( ret < 0 ) break;
		interrupted = (*env)->CallBooleanMethod( env, jobj, interrupt);
		if(interrupted) {
			return;
		}
...
#endif
			(*env)->CallVoidMethod( env, jobj, sendEvent, (jint)PAR_EV_ERROR, JNI_TRUE );
