...
RXTXPort.Initialize
   ...
   comments:    Basically this just causes rxtx to ignore signals.  signal
		...
		is enabled.
----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort (Initialize) (JNIEnv * env, jclass jclazz)
{
...
----------------------------------------------------------*/
int
find_preopened_ports (const char *filename)
{
  int fd;
...
----------------------------------------------------------*/
int
get_java_baudrate (int native_speed)
{
  switch (native_speed)
    {
    case B0:
...
void
set_java_vars (JNIEnv * env, jobject jobj, int fd)
{
  struct termios ttyset;
  int databits = -1;
  int jparity = -1;
...
RXTXPort.open
   ...
   comments:    Very often people complain about not being able to get past
                ...
                device file or bios has the device disabled.
----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort (open) (JNIEnv * env,
...
----------------------------------------------------------*/
int
set_port_params (JNIEnv * env, int fd, int cspeed, int dataBits,
...
----------------------------------------------------------*/
int
translate_speed (JNIEnv * env, jint speed)
{
...
----------------------------------------------------------*/
int
translate_data_bits (JNIEnv * env, tcflag_t * cflag, jint dataBits)
{
...
----------------------------------------------------------*/
int
translate_stop_bits (JNIEnv * env, tcflag_t * cflag, jint stopBits)
{
						   ...
						   jint fd)
{
  ...
  if ((ttyset.c_cflag & PARENB) &&
      (ttyset.c_cflag & PARODD) && (ttyset.c_cflag & CMSPAR))
    {
      return (JPARITY_MARK);
    }
...
----------------------------------------------------------*/
int
translate_parity (JNIEnv * env, tcflag_t * cflag, jint parity)
{
...
----------------------------------------------------------*/
void
finalize_threads (struct event_info_struct *eis)
{
#if     !defined(TIOCSERGETLSR) && !defined( WIN32 )
...
----------------------------------------------------------*/
int
init_threads (struct event_info_struct *eis)
{
  jfieldID jeis;
#if !defined(TIOCSERGETLSR) & !defined(WIN32)
...
fail:
  throw_java_exception (env, IO_EXCEPTION, "writeByte", strerror (errno));
}
...
JNIEXPORT void JNICALL RXTXPort (writeArray) (JNIEnv * env,
					      jobject jobj,
					      jbyteArray jbarray, jint offset,
					      jint count,
					      jboolean interrupted)
{
#ifndef TIOCSERGETLSR
...
fail:
  if (result < 0)
    throw_java_exception (env, IO_EXCEPTION, "writeArray", strerror (errno));
}
...
RXTXPort.NativeisReceiveTimeoutEnabled
   ...
   comments:   see  NativeEnableReceiveTimeoutThreshold
----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort (NativeisReceiveTimeoutEnabled) (JNIEnv *
								     env,
								     jobject
								     jobj)
{
...
void
static_add_filename (const char *filename, int fd)
{
...
RXTXPort.nativeGetBaudBase
   ...
   comments:    
		First introduced in rxtx-2.1-3
----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort (nativeGetBaudBase) (JNIEnv * env,
						     jobject jobj)
{
...
RXTXPort.nativeSetDivisor
   ...
   comments:    
		...
		First introduced in rxtx-2.1-3
----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort (nativeSetDivisor) (JNIEnv * env,
							jobject jobj,
							jint Divisor)
{
...
RXTXPort.nativeGetDivisor
   ...
   comments:    
		First introduced in rxtx-2.1-3
----------------------------------------------------------*/
JNIEXPORT jint JNICALL RXTXPort (nativeGetDivisor) (JNIEnv * env,
						    jobject jobj)
{
...
RXTXPort.nativeStaticSetSerialPortParams
   ...
   comments:    Not set the speed on the next 'open'
		...
		First introduced in rxtx-1.5-9
----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort (nativeStaticSetSerialPortParams) (JNIEnv *
								   env,
								   jclass
								   jclazz,
								   jstring
...
#ifdef WIN32
  result =
    (jbyte) termiosGetParityErrorChar (get_java_var (env, jobj, "fd", "I"));
#else
  /*
								...
								jbyte value)
{
  ...
  throw_java_exception (env, UNSUPPORTED_COMM_OPERATION,
			"Not implemented... yet", strerror (errno));
  LEAVE ("nativeSetParityErrorChar");
  return (JNI_FALSE);
#endif /* WIN32 */
}
...
int
read_byte_array (JNIEnv * env,
		 jobject * jobj,
		 int fd, unsigned char *buffer, int length, int timeout)
{
...
----------------------------------------------------------*/
JNIEXPORT void JNICALL RXTXPort (NativeEnableReceiveTimeoutThreshold) (JNIEnv
								       * env,
								       jobject
								       jobj,
								       jint
...
JNIEXPORT jint JNICALL RXTXPort (readArray) (JNIEnv * env,
					     jobject jobj, jbyteArray jbarray,
...
----------------------------------------------------------*/
int
check_line_status_register (struct event_info_struct *eis)
{
#ifdef TIOCSERGETLSR
  ...
  if (ioctl (eis->fd, TIOCSERGETLSR, &eis->change))
    ...
    {
      report_verbose
	("check_line_status_register: sending OUTPUT_BUFFER_EMPTY\n");
...
*/
  if (eis && eis->output_buffer_empty_flag == 1 &&
    ...
    {
      report_verbose
	("check_line_status_register: sending SPE_OUTPUT_BUFFER_EMPTY\n");
      send_event (eis, SPE_OUTPUT_BUFFER_EMPTY, 1);
/*
...
----------------------------------------------------------*/
int
has_line_status_register_access (int fd)
{
...
----------------------------------------------------------*/
void
check_cgi_count (struct event_info_struct *eis)
{
...
----------------------------------------------------------*/
int
port_has_changed_fionread (struct event_info_struct *eis)
{
  int change, rc;
  char message[80];
...
----------------------------------------------------------*/
void
check_tiocmget_changes (struct event_info_struct *eis)
{
...
----------------------------------------------------------*/
int
driver_has_tiocgicount (struct event_info_struct *eis)
{
#if defined(TIOCGICOUNT)
...
----------------------------------------------------------*/
void
report_serial_events (struct event_info_struct *eis)
{
...
----------------------------------------------------------*/
int
initialise_event_info_struct (struct event_info_struct *eis)
{
  int i;
...
----------------------------------------------------------*/
void
finalize_event_info_struct (struct event_info_struct *eis)
{
  if (eis->jclazz)
    {
...
JNIEXPORT jstring JNICALL RXTXCommDriver (nativeGetVersion) (JNIEnv * env,
							     jclass jclazz)
{
  return (*env)->NewStringUTF (env, "RXTX-2.0-7pre2");
}
...
----------------------------------------------------------*/
jboolean
is_interrupted (struct event_info_struct *eis)
{
  int result;
  JNIEnv *env = eis->env;
...
----------------------------------------------------------*/
int
send_event (struct event_info_struct *eis, jint type, int flag)
{
  int result;
  JNIEnv *env;
  if (eis)
...
----------------------------------------------------------*/
void
throw_java_exception (JNIEnv * env, char *exc, char *foo, char *msg)
{
  char buf[60];
...
----------------------------------------------------------*/
int
lfs_lock (const char *filename, int pid)
{
  int s;
  int ret;
  int size = 1024;
...
----------------------------------------------------------*/
int
lfs_unlock (const char *filename, int pid)
{
  int s;
  int ret;
  int size = 1024;
...
----------------------------------------------------------*/
int
fhs_lock (const char *filename, int pid)
{
  /*
...
----------------------------------------------------------*/
int
uucp_lock (const char *filename, int pid)
{
...
----------------------------------------------------------*/
int
check_lock_status (const char *filename)
{
  struct stat buf;
...
----------------------------------------------------------*/
void
fhs_unlock (const char *filename, int openpid)
{
  char file[80], *p;
  int i;
...
----------------------------------------------------------*/
void
uucp_unlock (const char *filename, int openpid)
{
  struct stat buf;
...
----------------------------------------------------------*/
int
check_lock_pid (const char *file, int openpid)
{
  int fd, lockpid;
  char pid_buffer[12];
  char message[80];
