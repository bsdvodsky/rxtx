...
----------------------------------------------------------*/
int
serial_test (char *filename)
{
  unsigned long *hcomm;
  int ret;
  hcomm =
    CreateFile (filename, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING,
		0, 0);
...
void
termios_setflags (int fd, int termios_flags[])
{
...
int
bytesize_to_termios (int ByteSize)
{
...
int
ClearErrors (struct termios_list *index, COMSTAT * Stat)
{
  ...
  if (ErrCode)
    {
      printf ("%i frame %i %i overrun %i %i  parity %u %i brk %i %i\n",
	      ...
	      index->sis->parity, (int) ErrCode & CE_BREAK, index->sis->brk);
    }
#endif /* DEBUG_ERRORS */
  if (ErrCode & CE_FRAME)
    {
...
#ifdef LIFE_IS_GOOD
  FIXME OVERRUN is spewing if (ErrCode & CE_OVERRUN)
    {
      index->sis->overrun++;
      ErrCode &= ~CE_OVERRUN;
    }
...
BOOL
FillDCB (DCB * dcb, unsigned long *hCommPort, COMMTIMEOUTS Timeout)
{
...
void
cfmakeraw (struct termios *s_termios)
{
  ENTER ("cfmakeraw");
...
BOOL
init_serial_struct (struct serial_struct *sstruct)
{
...
BOOL
init_termios (struct termios * ttyset)
{
  ENTER ("init_termios");
  if (!ttyset)
    return FALSE;
...
int
port_opened (const char *filename)
{
  struct termios_list *index = first_tl;
...
int
open_port (struct termios_list *port)
{
  ENTER ("open_port");
...
int
check_port_capabilities (struct termios_list *index)
{
  COMMPROP cp;
  DCB dcb;
  char message[80];
...
int
serial_open (const char *filename, int flags, ...)
{
  struct termios_list *index;
  char message[80];
...
int
serial_write (int fd, char *Str, int length)
{
...
int
serial_read (int fd, void *vb, int size)
{
  long start, now;
...
*/
	    ...
	    case ERROR_IO_PENDING:
	      while (!GetOverlappedResult (index->hComm,
					   &index->rol, &nBytes, TRUE))
		{
		  if (GetLastError () != ERROR_IO_INCOMPLETE)
		    {
		      ClearErrors (index, &stat);
...
int
cfsetospeed (struct termios *s_termios, speed_t speed)
{
  char message[80];
  ENTER ("cfsetospeed");
  if (speed & ~CBAUD)
...
int
cfsetispeed (struct termios *s_termios, speed_t speed)
{
...
int
cfsetspeed (struct termios *s_termios, speed_t speed)
{
  return cfsetospeed (s_termios, speed);
}
...
speed_t
cfgetospeed (struct termios * s_termios)
{
  ENTER ("cfgetospeed");
...
speed_t
cfgetispeed (struct termios * s_termios)
{
  ENTER ("cfgetospeed");
...
----------------------------------------------------------*/
int
serial_struct_to_DCB (struct serial_struct *sstruct, DCB * dcb)
{
  /* 5 Baud rate fix
     sstruct.baud_base
...
----------------------------------------------------------*/
int
termios_to_DCB (struct termios *s_termios, DCB * dcb)
{
...
----------------------------------------------------------*/
int
DCB_to_serial_struct (DCB * dcb, struct serial_struct *sstruct)
{
  return (0);
}
...
----------------------------------------------------------*/
void
DCB_to_termios (DCB * dcb, struct termios *s_termios)
{
...
show_DCB (myDCB)
{
  ...
  switch (myDCB.Parity)
    ...
    case ODDPARITY:
      ...
      break;
    default:
      sprintf (message, "unknown Parity (%#x ):", myDCB.Parity);
      report (message);
      break;
    }
  report ("\n");
...
int
tcgetattr (int fd, struct termios *s_termios)
{
  DCB myDCB;
...
----------------------------------------------------------*/
int
tcsetattr (int fd, int when, struct termios *s_termios)
{
  int vtime;
  DCB dcb;
  COMMTIMEOUTS timeouts;
...
----------------------------------------------------------*/
void
termios_interrupt_event_loop (int fd, int flag)
{
  struct termios_list *index = find_port (fd);
  if (!index)
    {
...
#ifndef __LCC__
int
serial_select (int n, fd_set * readfds, fd_set * writefds,
	       fd_set * exceptfds, struct timeval *timeout)
{
  ...
  usleep (1000) LEAVE ("serial_select");
  return (1);
timeout:
  LEAVE ("serial_select");
  return (0);
...
*/
	...
	case WAIT_OBJECT_0:
	  ...
	  if (!index->sol.hEvent)
	    return (1);
	  if (!GetOverlappedResult (index->hComm, &index->sol, &nBytes, TRUE))
	    {
	      goto end;
	    }
...
int
termiosGetParityErrorChar (int fd)
{
  struct termios_list *index;
  DCB dcb;
...
void
termiosSetParityError (int fd, char value)
{
  DCB dcb;
  struct termios_list *index;
...
{
  unsigned char value;
  __asm__ volatile ("inb %1,%0":"=a" (value):"d" ((unsigned short) index));
  return value;
}

static inline void
outportb (unsigned char val, unsigned short int index)
{
