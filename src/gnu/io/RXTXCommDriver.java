	...
	if (debug)
	  {
	    System.out.
	      ...
	      ("\nRXTXCommDriver:getValidPortPrefixes() No ports matched the list assumed for this\nSystem in the directory "
	       + deviceDirectory + ".  Please check the ports listed for \"" +
	       osName +
...
*/
  ...
  private boolean registerSpecifiedPorts (int PortType)
  {
	...
	{
	  System.out.
	    println ("The file: javax.comm.rxtx.properties doesn't exists.");
	...
	if ((val =
	     System.getProperty ("javax.comm.rxtx.SerialPorts")) == null)
	  val = System.getProperty ("javax.comm.SerialPorts");
	break;

      case CommPortIdentifier.PORT_PARALLEL:
	if ((val =
	     System.getProperty ("javax.comm.rxtx.ParallelPorts")) == null)
  ...
  private void registerScannedPorts (int PortType)
  {
	...
	if (debug)
	  System.out.
	    println
	    ("RXTXCommDriver:registerScannedPorts() no Device files to check ");
	return;
      }
	...
	if (osName.equals ("Linux"))
	...
	else if (osName.equals ("OpenServer"))
	  {
		...
		"tty2A", "tty3A", "tty4A", "tty5A", "tty6A", "tty7A", "tty8A", "tty9A", "tty10A", "tty11A", "tty12A", "tty13A", "tty14A", "tty15A", "tty16A", "ttyu1A",	// OSR5 USB-serial ports
	    "ttyu2A",
		"ttyu3A",
		"ttyu4A",
		"ttyu5A",
	...
	else
	  {
	    if (debug)
	      System.out.
		println
		("No valid prefixes for serial ports have been entered for " +
		 osName +
