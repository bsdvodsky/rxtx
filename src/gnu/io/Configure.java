...
class Configure extends Frame {
	...
	private void saveSpecifiedPorts() {
		...
		if (PortType == PORT_SERIAL)
			filename = new String(javaHome + fileSep + "lib" + fileSep
					+ "javax.comm.rxtx.SerialPorts");
		else if (PortType == PORT_PARALLEL)
			filename = new String(javaHome + "javax.comm.rxtx.ParallelPorts");
		else {
		...
		try {
			...
			for (int i = 0; i < 128; i++) {
				if (cb[i].getState()) {
					output = new String(cb[i].getLabel() + pathSep);
	...
	public Configure() {
		...
		int Height = 480;
		cb = new Checkbox[128];
		final Frame f = new Frame("Configure javax.comm.rxtx.properties");
		String fileSep = System.getProperty("file.separator", "/");
		String devPath;
